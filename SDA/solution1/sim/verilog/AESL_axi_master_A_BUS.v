...
//------------------------Parameter----------------------
`define TV_IN_A_BUS "../tv/cdatafile/c.top.autotvin_A_BUS.dat"
`define TV_OUT_A_BUS "../tv/rtldatafile/rtl.top.autotvout_A_BUS.dat"
 parameter A_BUS_ADDR_BITWIDTH = 32'd 64;
 parameter A_BUS_AWUSER_BITWIDTH = 32'd 1;
 parameter A_BUS_DATA_BITWIDTH = 32'd 512;
...
parameter A_BUS_C_DATA_BITWIDTH = 32'd 512;
parameter A_BUS_transaction_depth = 32'd 5000;
parameter A_BUS_mem_depth = 32'd 5000;
...
reg    BRESP_tmp = 0;
reg    [A_BUS_ID_BITWIDTH - 1 : 0] BID_tmp = 0;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [A_BUS_ID_BITWIDTH - 1 : 0] RID_tmp = 0;
reg [A_BUS_DATA_BITWIDTH - 1 : 0] A_BUS_mem_0 [0: A_BUS_mem_depth - 1] = '{default : 'h0}; 
reg [A_BUS_DATA_BITWIDTH - 1 : 0] A_BUS_mem_1 [0: A_BUS_mem_depth - 1] = '{default : 'h0}; 
reg [A_BUS_DATA_BITWIDTH - 1 : 0] A_BUS_mem_2 [0: A_BUS_mem_depth - 1] = '{default : 'h0}; 
reg [A_BUS_DATA_BITWIDTH - 1 : 0] A_BUS_mem_3 [0: A_BUS_mem_depth - 1] = '{default : 'h0}; 
...
always @ (posedge clk or reset) begin 
    if(reset === 0) begin 
        ...
        if(TRAN_A_BUS_AWREADY & TRAN_A_BUS_AWVALID ) begin    // RTL is sending a AW_request to the bus         
            ...
            FIFO_AW_req_ID[FIFO_AW_req_ptr_w]    =   TRAN_A_BUS_AWID; 
...
always @ (posedge clk or reset) begin 
    if(reset === 0) begin 
        ...
        if(TRAN_A_BUS_ARREADY & TRAN_A_BUS_ARVALID ) begin    // RTL is sending a AR_request to the bus         
            ...
            FIFO_AR_req_ID[FIFO_AR_req_ptr_w]    =   TRAN_A_BUS_ARID; 
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_A_BUS_AWVALID && TRAN_A_BUS_AWREADY && ((FIFO_AW_req_ptr_w - FIFO_AW_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
          FIFO_AW_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_A_BUS_ARVALID && TRAN_A_BUS_ARREADY && ((FIFO_AR_req_ptr_w - FIFO_AR_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
          FIFO_AR_req_flag <= 1; 
      # 0.4;
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_A_BUS_WVALID && TRAN_A_BUS_WREADY && ((FIFO_WDATA_req_ptr_w - FIFO_WDATA_req_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
...
always @(posedge clk or reset) begin 
    if(reset === 0) begin 
      ...
      if(TRAN_A_BUS_WVALID && TRAN_A_BUS_WREADY && TRAN_A_BUS_WLAST && ((FIFO_WDATA_size_ptr_w - FIFO_WDATA_size_ptr_r) % FIFO_DEPTH == FIFO_DEPTH - 1)) 
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > A_BUS_mem_depth) begin
                $display ("D:/Download/SDA/SDA/solution1/sim/verilog/AESL_axi_master_A_BUS.v: Write request address %d exceed AXI master A_BUS array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, A_BUS_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > A_BUS_mem_depth) begin
                $display ("D:/Download/SDA/SDA/solution1/sim/verilog/AESL_axi_master_A_BUS.v: Read request address %d exceed AXI master A_BUS array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, A_BUS_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < A_BUS_transaction_depth; i = i + 1) begin 
          ...
          if (i%1 == 0) begin
              case(mem_page)
                  0 : A_BUS_mem_0[i/1] = mem_tmp;
                  1 : A_BUS_mem_1[i/1] = mem_tmp;
                  2 : A_BUS_mem_2[i/1] = mem_tmp;
                  3 : A_BUS_mem_3[i/1] = mem_tmp;
