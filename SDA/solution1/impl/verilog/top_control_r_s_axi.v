...
//------------------------Parameter----------------------
localparam
    ADDR_CONV_MM_A_DATA_0   = 6'h10,
    ADDR_CONV_MM_A_DATA_1   = 6'h14,
    ADDR_CONV_MM_A_CTRL     = 6'h18,
    ADDR_CONV_WEIGHT_DATA_0 = 6'h1c,
    ADDR_CONV_WEIGHT_DATA_1 = 6'h20,
    ADDR_CONV_WEIGHT_CTRL   = 6'h24,
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (ar_hs) begin
            rdata <= 'b0;
            case (raddr)
                ADDR_CONV_MM_A_DATA_0: begin
                    rdata <= int_Conv_MM_A[31:0];
                end
                ADDR_CONV_MM_A_DATA_1: begin
                    rdata <= int_Conv_MM_A[63:32];
                end
                ADDR_CONV_WEIGHT_DATA_0: begin
                    rdata <= int_Conv_Weight[31:0];
                end
                ADDR_CONV_WEIGHT_DATA_1: begin
                    rdata <= int_Conv_Weight[63:32];
                end
                ADDR_MM_WEIGHT_DATA_0: begin
                    rdata <= int_MM_Weight[31:0];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_Conv_MM_A[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CONV_MM_A_DATA_0)
            int_Conv_MM_A[31:0] <= (WDATA[31:0] & wmask) | (int_Conv_MM_A[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_Conv_MM_A[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CONV_MM_A_DATA_1)
            int_Conv_MM_A[63:32] <= (WDATA[31:0] & wmask) | (int_Conv_MM_A[63:32] & ~wmask);
    end
end

// int_Conv_Weight[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_Conv_Weight[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CONV_WEIGHT_DATA_0)
            int_Conv_Weight[31:0] <= (WDATA[31:0] & wmask) | (int_Conv_Weight[31:0] & ~wmask);
    end
end

// int_Conv_Weight[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_Conv_Weight[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CONV_WEIGHT_DATA_1)
            int_Conv_Weight[63:32] <= (WDATA[31:0] & wmask) | (int_Conv_Weight[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_MM_Weight[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_MM_WEIGHT_DATA_0)
            int_MM_Weight[31:0] <= (WDATA[31:0] & wmask) | (int_MM_Weight[31:0] & ~wmask);
    end
end
