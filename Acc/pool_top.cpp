...
void pool_top(DataInput *weight, DataInput *INPUT_BUF, DataOutput *OUTPUT_BUF, DataType *BIAS_BUF, unsigned r, unsigned c, unsigned n, unsigned k, unsigned s, unsigned p, unsigned pool_type)
{
...
#pragma HLS ARRAY_PARTITION variable = out_buf_1 dim = 3 complete
    ...
    DataInput in_buf_0[POOL_S * Tr + POOL_K - POOL_S][POOL_S * Tc + POOL_K - POOL_S];
    DataInput in_buf_1[POOL_S * Tr + POOL_K - POOL_S][POOL_S * Tc + POOL_K - POOL_S];
...
#pragma HLS LOOP_TRIPCOUNT max = CONV_TEST_OUT_R / Tr * CONV_TEST_OUT_C / Tc * CONV_TEST_M / MAX_INP min = CONV_TEST_OUT_R / Tr * CONV_TEST_OUT_C / Tc * CONV_TEST_M / MAX_INP
        if (flag)
        {
            fillZeros(out_buf_1, bias_buf_1, pool_type);
            loadIn(INPUT_BUF, in_buf_0, r, c, n, k, s, p, row, col, nn);
            loadWeight(weight, w_buf_0, k, n, nn, pool_type);
            loadBias(BIAS_BUF, bias_buf_0, nn, pool_type);
            compute(in_buf_1, w_buf_1, out_buf_1, k, s, pool_type, trans1);
            storeOut(out_buf_0, OUTPUT_BUF, out_c, n, sto_row, sto_col, sto_nn, trans2);
        }
        else
        {
            fillZeros(out_buf_0, bias_buf_0, pool_type);
            loadIn(INPUT_BUF, in_buf_1, r, c, n, k, s, p, row, col, nn);
            loadWeight(weight, w_buf_1, k, n, nn, pool_type);
            loadBias(BIAS_BUF, bias_buf_1, nn, pool_type);
            compute(in_buf_0, w_buf_0, out_buf_0, k, s, pool_type, trans1);
...
#pragma HLS LOOP_TRIPCOUNT max = CONV_TEST_S * Tc + CONV_TEST_K - CONV_TEST_S min = CONV_TEST_S * Tc + CONV_TEST_K - CONV_TEST_S
            ...
            {
                in_buf[i][j](MAX_INP * BIT - 1, 0) = DataInput(0)(MAX_INP * BIT - 1, 0);
            }
            else
            {
                DataInput temp = *(RES_BUF + (r_base + i) * c * (n >> INP_LOG2_BIT) + (c_base + j) * (n >> INP_LOG2_BIT) + (nn >> INP_LOG2_BIT));
...
#pragma HLS LOOP_TRIPCOUNT max = CONV_TEST_K min = CONV_TEST_K
        ...
        {
#pragma HLS LOOP_TRIPCOUNT max = CONV_TEST_K min = CONV_TEST_K
            DataInput temp = *(weight + i * k * (n >> INP_LOG2_BIT) + j * (n >> INP_LOG2_BIT) + (nn >> INP_LOG2_BIT));
...
void storeOut(DataType out_buf[Tr][Tc][MAX_INP], DataOutput *RES_BUF, unsigned out_c, unsigned n, unsigned row, unsigned col, unsigned nn, bool trans)
{
    ...
    for (unsigned i = 0; i < Tr; i++)
    {
        for (unsigned j = 0; j < Tc; j++)
        {
            ...
            out_base = RES_BUF + (row + i) * out_c * (n >> 4) + (col + j) * (n >> 4) + (nn >> 4);
            *out_base = out1;
            *(out_base + 1) = out2;
        }
    }
