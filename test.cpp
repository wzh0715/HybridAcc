...
void MMTest()
{
    ...
    ap_uint<MAX_INP * BIT> conv3_weight[CONV_TEST_K * CONV_TEST_K * CONV_TEST_N * CONV_TEST_M / MAX_INP];
    float O[MM_TEST_R * MM_TEST_M];
    ...
    top((ap_uint<MAX_INP * BIT> *)A_pack, (ap_uint<MAX_INP * BIT> *)conv3_weight, (ap_uint<MAX_OUP * BIT> *)W_pack, (float *)bias, (ap_uint<NORM_BIT> *)norm_pack, (float *)O, MM_TEST_R, 0, MM_TEST_N, MM_TEST_M, 0, 0, 0, false);
    ...
    for (int i = 0; i < MM_TEST_R; i++)
    {
        ...
        {
            if (O[i * MM_TEST_M + j] != O_golden[i][j])
            {
                std::cout << "[r][m]: " << i << " " << j << " " << "\t" << "out : " << O[i * MM_TEST_M + j] << "\t\t" << "ref : " << O_golden[i][j] << std::endl;
            }
        }
    }
}
...
void ConvTest()
{
    ...
    float conv3_res[CONV_TEST_OUT_R * CONV_TEST_OUT_C * CONV_TEST_M];
    ap_uint<MAX_OUP * BIT> W_pack[1];
    top((ap_uint<MAX_INP * BIT> *)conv_A, (ap_uint<MAX_INP * BIT> *)conv3_weight_re, (ap_uint<MAX_OUP * BIT> *)W_pack, (float *)bias, (ap_uint<NORM_BIT> *)norm_pack, (float *)conv3_res, CONV_TEST_R, CONV_TEST_C, CONV_TEST_N, CONV_TEST_M, CONV_TEST_K, CONV_TEST_P, CONV_TEST_S, true);
    ...
    for (int k = 0; k < CONV_TEST_M; k++)
    {
        for (int i = 0; i < CONV_TEST_OUT_R; i++)
        {
            ...
            {
                if(std::abs(conv3_res[i * CONV_TEST_OUT_C * CONV_TEST_M + j * CONV_TEST_M + k] - conv_gloden[i][j][k]) > EPSILON)
                {
                    std::cout << std::scientific << std::setprecision(std::numeric_limits<float>::max_digits10);
                    std::cout << "[r][c][m]: " << i << " " << j << " " << k << "\t" << "out : " << conv3_res[i * CONV_TEST_OUT_C * CONV_TEST_M + j * CONV_TEST_M + k] << "\t\t" << "ref : " << conv_gloden[i][j][k] << std::endl;
                }
            }
        }
    }
