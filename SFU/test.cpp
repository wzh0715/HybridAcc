...
void MMTest()
{
    ...
    ap_uint<NORM_BIT> norm_pack[MM_TEST_M];
...
#ifdef MM_NORM_DEBUG
    sfu_top(A_pack, res, norm_pack, MM_TEST_R, 0, MM_TEST_M, 0, false, false);
    mm_norm(A, res_golden, norm);
#endif
#ifdef MM_SOFTMAX_DEBUG
    sfu_top(A_pack, res, norm_pack, MM_TEST_R, 0, MM_TEST_M, 3, false, false);
    mm_softmax(A, res_golden);
#endif
    ...
    for (int i = 0; i < MM_TEST_R; i++)
    {
        for (int j = 0; j < MM_TEST_M / MAX_OUP; j++)
        {
            ...
            for (int k = 0; k < MAX_OUP; k++)
            {
                ...
                {
                    std::cout << "[r][m]: " << i << " " << j * MAX_OUP + k << " " << "\t" << "out : " << temp << "\t\t" << "ref : " << res_golden[i][j * MAX_OUP + k] << std::endl;
                }
            }
        }
    }
...
void ConvTest()
{
    ...
    sfu_top(A_pack, res, norm_pack, CONV_TEST_R, CONV_TEST_C, CONV_TEST_M, 0, false, true);
    ...
    sfu_top(A_pack, res, norm_pack, CONV_TEST_R, CONV_TEST_C, CONV_TEST_M, 2, false, true);
...
int main()
{
#ifdef MM_NORM_DEBUG || MM_SOFTMAX_DEBUG
    MMTest();
#endif
#ifdef CONV_NORM_DEBUG || CONV_RELU_DEBUG || CONV_SIGMOID_DEBUG
    ConvTest();
#endif
    return 0;
}